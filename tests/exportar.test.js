...
// (un empalme no es apoyo → el vano real va de estructura a estructura), la
// prueba verifica la divergencia exacta, no la ignora.
//
...
import { aGMS } from '../web/src/vistas/gms.js';
import { derivarLevantamiento, horaLocalBogota } from '../web/src/exportar/levantamiento.js';
import { generarGpx } from '../web/src/exportar/gpx.js';
import { generarKml } from '../web/src/exportar/kml.js';
import { generarCsv, COLUMNAS_CSV } from '../web/src/exportar/csv.js';
...
const LINEA = { codigo: 'LN-627', nombre: 'Línea LN-627' };
const filasCsv = (csv) => csv.replace('﻿', '').split('\r\n').slice(2).map((f) => f.split(';'));
...
describe('exportadores — contra la tabla del módulo original', () => {
  ...
  test('GPX: 26 waypoints + 26 puntos de traza, precisiones del original', { skip: SIN_BOVEDA }, () => {
    const gpx = generarGpx(LINEA, lev);
  ...
  test('CSV: donde no hay empalme en medio, vano y azimut = los del original', { skip: SIN_BOVEDA }, () => {
    ...
    for (let i = 1; i < crudo.length; i++) {
      ...
      cerca(Number(filas[i][cVano]), crudo[i].d, 0.01, `vano del punto ${i + 1}`);
      cerca(Number(filas[i][cAz]), crudo[i].az, 0.01, `azimut del punto ${i + 1}`);
      comparados++;
    }
  ...
  test('CSV: la divergencia deliberada — el vano real NO se parte en los empalmes', { skip: SIN_BOVEDA }, () => {
    ...
    for (const i of [5, 7]) {
      assert.equal(filas[i][cTipo], '"Empalme"');
      assert.equal(filas[i][cVano], '', 'un empalme no corta ni tiene vanos');
      assert.ok(filas[i][cEnVano].includes('→'), 'declara dentro de qué vano vive');
    }
    ...
    cerca(Number(filas[8][cVano]), directo, 0.01, 'vano real E06→E07 (el CSV redondea a 2 decimales)');
  ...
  test('progresivas: iguales al original hasta el primer empalme; el total es la línea', { skip: SIN_BOVEDA }, () => {
    ...
    for (let i = 0; i < 5; i++) {           // E01..E05: aún no hay empalmes en medio
      cerca(Number(filas[i][cProg]), crudo[i].acc, 0.01, `progresiva del punto ${i + 1}`);
    }
    const ultima = Number(filas[25][cProg]);
    ...
    cerca(ultima, sumaVanos, 0.001, 'y coincide con la suma de los 23 vanos reales');
  });

  test('KML: tramos de tensión con la traza completa y empalmes en su carpeta', { skip: SIN_BOVEDA }, () => {
    const kml = generarKml(LINEA, lev);
...
describe('exportadores — fronteras del estado cero (sin bóveda también corren)', () => {
  test('sin apoyos: documentos válidos y vacíos, sin explotar', () => {
    ...
    for (const texto of [generarGpx({ codigo: 'X' }, lev), generarKml({ codigo: 'X' }, lev), generarCsv(lev)]) {
