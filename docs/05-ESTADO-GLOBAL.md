...
|---|---|
| **Misión ahora** | **Llenar la ficha estructural, que YA se puede escribir** (17-08, `99 §ADR-030`, en producción). El motor dictamina desde julio y los apoyos seguían sin veredicto porque no había por dónde meter el dato: con 3 campos declarados en un apoyo, su veredicto aparece. La pregunta que queda es del Ingeniero: ¿esas fichas están en planos y actas, o hay que levantarlas en campo? (`TODO-57`). Herramienta INTERNA, sin cliente ni contrato (`ADR-022`). Pendientes → `10`. |
| **Build** | 🟢 `npm test` **1.476 pass / 0 fail** (núcleo · exportes · vistas · RCA · contrato · identidad de los apoyos · ficha editable · **la marca de dato SUPUESTO, dos ejes**). `contrato:verificar` exit 0, **contrato v0.6.0**. · verificado-vivo: 2026-08-19 |
| **Repo** | `main` · https://github.com/PowertransformersMJ/mantenimiento-lineas-at · **PÚBLICO** (decisión del Ingeniero 2026-07-29) |
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages, cuenta `ajimenezp99@gmail.com`. **Piel CLARA** desde el 04-08 («El Horizonte» F3): contraste medido, 0 bajo el mínimo. **Se despliega a mano** (`build` delante: `deploy` NO construye, `32 · L-35`); el automático espera 2 secretos. **Contrato v0.6.0** con la ficha editable dentro — verificado el 19-08 en el bundle servido, NO con su Chrome (prueba lo que se sirve, no lo que él ve al entrar). · verificado-vivo: 2026-08-19 |
...
| **Deuda crítica** | 🔴 **El DATO de la ficha de los 24 apoyos** (`TODO-57`): la pantalla para meterlo ya existe (`ADR-030`), el dato no. ✅ **Los papeles ya marcan los veredictos sobre datos SUPUESTOS** (`ADR-032`, 19-08): lo decide el MOTOR, eje por eje. 🟡 Dónde deben vivir los datos del empleador (`TODO-58`). (✅ motor sin deuda desde 07-29, `40 §8` · ✅ Consejo Externo, ADR-002 · ✅ alcance y presupuesto, ADR-003 · ✅ contexto real fijado, **ADR-022**: sin cliente ni contrato — `TODO-02` retirado por falta de objeto.) |
