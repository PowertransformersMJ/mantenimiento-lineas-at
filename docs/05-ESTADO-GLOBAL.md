...
|---|---|
| **Misión ahora** | **F0 · Verificación.** Repo creado, cerebro cableado, `nucleo/` portado y verificado. Arquitectura decidida por comité ×3 (ADR-001) — **falta Consejo Externo antes de escribir aplicación**. Pendientes → `10`. |
...
| **Deuda crítica** | 🔴 (a) **Consejo Externo sin correr** → ADR-001 marcado *NO revisado externamente*; prompt listo en la bóveda. (b) **Cambio de estado y vano peso sin contrastar contra caso resuelto de norma** (`40 §8`) → bloquea emitir cálculo con valor de entrega. (c) **8 preguntas a AFINIA sin enviar**: la nº 2 puede recortar el 70 % del alcance. |
