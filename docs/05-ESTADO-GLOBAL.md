...
| **Build** | 🟢 `npm test` **1.745 pass / 0 fail** · `contrato:verificar` exit 0, **contrato v0.8.0** · motor v0.3.0 · verificado-vivo: 2026-08-22 |
| **Repo** | `main` · https://github.com/PowertransformersMJ/mantenimiento-lineas-at · **PÚBLICO** (decisión del Ingeniero 2026-07-29) |
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages, cuenta `ajimenezp99@gmail.com`. **Piel CLARA** desde el 04-08. **Se despliega a mano**, `build` delante: `deploy` NO construye (`32 · L-35`). Último: **el número que se firma** —un solo tope de puesta a tierra y el pendiente del despeje que por fin se tacha (`ADR-052`)—, tras las señales de la página (`ADR-051`) y el atlas solar (`ADR-045/046`). ⏳ **sin verificar en vivo todavía.** · verificado-vivo: 2026-08-22 |
...
| **Coste** | **$0/mes.** R2: 10 GB gratis, ~35 MB usados (tarifa oficial 2026-08-03). Pendiente: alerta de gasto. **Criterio:** se prefiere el servicio que APAGA al que COBRA (`31 · L-02`). |
| **Cerebro** | kernel íntegro == canónico (versión → `brain:check`) · bóveda con crudo del comité (477 KB) y fixture de LN-627 |
| **Alcance** | **Plataforma COMPLETA e independiente** (ADR-003): no se integra con Maximo ni SAP, trae todo lo necesario. F5 (sincronización campo↔oficina) deja de ser condicional. |
| **Deuda crítica** | 🔴 **El DATO de la ficha de los 25 apoyos** (`TODO-57`): la pantalla existe —una a una y por lote—, el dato no; y el lote solo trae el tercio de catálogo (`33 · L-59`). 🟡 Dónde deben vivir los datos del empleador (`TODO-58`) · 🟡 **viento y los 1.000 W/m² de la ampacidad siguen ADOPTADOS**: piden serie histórica (`TODO-71`). (ya cerradas: motor sin deuda `40 §8` · ADR-002/003/022/032.) |
