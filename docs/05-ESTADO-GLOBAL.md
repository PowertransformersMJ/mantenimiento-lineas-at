...
| **Build** | 🟢 `npm test` **1.607 pass / 0 fail** · `contrato:verificar` exit 0, **contrato v0.6.0** · verificado-vivo: 2026-08-20 |
| **Repo** | `main` · https://github.com/PowertransformersMJ/mantenimiento-lineas-at · **PÚBLICO** (decisión del Ingeniero 2026-07-29) |
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages, cuenta `ajimenezp99@gmail.com`. **Piel CLARA** desde el 04-08. **Se despliega a mano** y `build` va delante: `deploy` NO construye (`32 · L-35`). Último: **temperatura del aire** (`ADR-039`) y **satelital remuestreada al nivel 15** (`ADR-040`), las dos recorridas en vivo con su sesión. Antes: la pantalla del lote (`ADR-038`). **Hash servido == construido.** · verificado-vivo: 2026-08-20 |
