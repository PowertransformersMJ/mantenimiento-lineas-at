...
| **Build** | 🟢 `npm test` **2.468 pass / 0 fail** + emulador 47/47 · `contrato:verificar` exit 0, **contrato v0.15.0** · motor v0.13.0 · verificado-vivo: 2026-09-06 |
...
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages (`ajimenezp99@gmail.com`). **A mano**, `build` delante: `deploy` NO construye (`35 · L-35`). La línea: **30 puntos · 26 estructuras · 3.032,8 m** (`ADR-054`). El clima vive en el Atlas, con **el trazado** y **de cuándo es el dato** (`ADR-055..087`). **ONCE capas** en 3 familias: cinco del año (POWER), tres del satélite GOES que se ACUMULAN y **tres de PRONÓSTICO**, modelo y con caducidad (`ADR-079/081/086`). Medido: temperatura, viento y lluvia al **2026-08-23**; sol y nubes al **2026-05-30** (tope de su fuente); satélite al minuto; pronóstico al **04-09**. **hash y cinta verificados en vivo con su sesión** · verificado-vivo: 2026-08-26 |
| **Backend** | 🟢 **Dos trabajadores**: el portero de fotos (`evidencias/`, ADR-010) y el de personas (`usuarios/`, ADR-100, **sin desplegar**). Datos en **Firestore** con RBAC por *claims*; **el ingreso exige contraseña, sin registro público** (ADR-019). **Residencia aceptada, São Paulo** · 🔒 **freno de borrado PUESTO** · ⚠️ sin copia (30-08, `ADR-089`). |
