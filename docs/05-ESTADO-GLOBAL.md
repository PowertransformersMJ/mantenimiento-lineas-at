...
| **Build** | 🟢 `npm test` **3.326 / 0 fail** + reglas 94/94 · contrato **v0.17.0** · motor v0.21.0 · verificado-vivo: 2026-09-20 |
...
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages (`ajimenezp99@gmail.com`). **A mano**: `pull`→`build`→`deploy` (`35 · L-35/77`), y las REGLAS aparte (`35 · L-22`). El parque: **3 líneas** — LN-627 (30 puntos · 26 estructuras · 3.032,8 m, `ADR-054`) y LN-617/LN-628, de alta el 20-09 sobre el **tramo 618**: 28 puntos levantados, sin torres ni conductor (`ADR-133`). El clima, en el Atlas (**ONCE capas**, `10 §3`); la fecha de cada una la dice su JSON (`30 · M-01`). **Parámetros eléctricos: enero-agosto, 208 días** (`ADR-128`): sus 8 señales validadas hora a hora del SCADA a la base el 11-09. · verificado-vivo: 2026-09-20 (hash servido == construido; atlas al día) |
