...
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages (`ajimenezp99@gmail.com`). **A mano**, `build` delante: `deploy` NO construye (`35 · L-35`). La línea: **30 puntos · 26 estructuras · 3.032,8 m** (`ADR-054`). El clima vive en el Atlas —**ONCE capas**, su arquitectura en `10 §3`—, y **la fecha de cada una la dice su propio JSON** (`30 · M-01`). **Parámetros eléctricos: enero-agosto, 208 días** (`ADR-128`), validados celda a celda el 11-09. · verificado-vivo: 2026-09-06 (hash servido == construido; CSP sin Google) |
