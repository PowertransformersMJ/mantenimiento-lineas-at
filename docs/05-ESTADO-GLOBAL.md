...
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages, cuenta `ajimenezp99@gmail.com`. **Piel CLARA** desde el 04-08. **Se despliega a mano**, `build` delante: `deploy` NO construye (`32 · L-35`). Último: **el número que se firma** —un tope de tierra con un solo dueño; el pendiente del despeje ya se tacha (`ADR-052`)—, tras `ADR-051`. **Recorrido en vivo con su sesión:** motor v0.3.0 y contrato v0.8.0 servidos, la fila de tierra ya no se atribuye al RETIE y la ficha publica su procedencia. **Hash servido == construido.** · verificado-vivo: 2026-08-22 |
