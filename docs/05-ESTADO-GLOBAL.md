...
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages, cuenta `ajimenezp99@gmail.com`. **Piel CLARA** desde el 04-08. **Se despliega a mano**, `build` delante: `deploy` NO construye (`32 · L-35`). Último: **los dos atlas del Caribe** —sol y TEMPERATURA, un motor, y ambos dentro de Detalle GPS sin pesar hasta que se abren (`ADR-053`)—, tras el número que se firma (`ADR-052`). ⏳ **`ADR-053` sin recorrer en vivo.** · verificado-vivo: 2026-08-22 |
