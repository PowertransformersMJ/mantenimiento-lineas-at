...
| **Producción** | 🟢 **https://mantenimiento-lineas-at.pages.dev** — Cloudflare Pages (`ajimenezp99@gmail.com`). **A mano**: `pull`→`build`→`deploy` (`35 · L-35/77`), y las REGLAS aparte (`35 · L-22`). El parque: **3 líneas** — LN-627 (30 puntos · 26 estructuras · 3.032,8 m, `ADR-054`) y LN-617/LN-628, de alta el 20-09 sobre el **tramo 618**: 28 puntos, sin torres ni conductor (`ADR-133`). El clima, en el Atlas (**ONCE capas**, `10 §3`); cada fecha la dice su JSON (`30 · M-01`). **Parámetros eléctricos: LAS TRES con ene-ago** (`ADR-128/135`) — LN-627 208 días (validada hora a hora el 11-09), LN-617 779 docs y LN-628 767, cargadas el 22-09. · verificado-vivo: 2026-09-22 (hash servido == construido; atlas al tope de su fuente, `ADR-136`) |
