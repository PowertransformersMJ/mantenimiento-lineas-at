...
| **TODO-52** | **RCA, lo que falta.** ✅ El camino de edición ya valida (`ParteDeAnalisis`, strict) — era el requisito previo a tocar nada más: la creación validaba y las ediciones NO. ⬜ **acciones CAPA**: van a colección propia `acciones_capa` con reglas nuevas, NO dentro del análisis (dentro de un array las reglas no distinguen «cerrar una acción» de «reescribir el razonamiento tras firmar») · ⬜ informe del análisis con sus límites impresos · ⬜ persistir `SondeoClima` (hoy la consulta se pierde al recargar, y sin ella el informe no puede imprimir el clima) · ⬜ lienzo del árbol | `99 §ADR-020` |
