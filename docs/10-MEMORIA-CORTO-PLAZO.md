...
| **TODO-82/83/85** | **Tres decisiones suyas sobre el clima.** ① ¿FASE 2 del pronóstico (`§ADR-057`): franja mañana/tarde y sensación térmica? ② ¿Dato FINO por extremos (`§ADR-064`)? Las capas de 2 km tocan **3 celdas** y salen por PROMEDIO. ③ ✅ HECHO (`§ADR-069/070`). ④ **`TODO-87`: al Atlas «Radiación solar» y «Temperatura ambiente»** (22-08): capas FINAS del corredor, OTRO subsistema (~400 líneas); lo único que sigue en Detalle GPS. Y `TODO-86`: retirar `ClimaDelAnio.tsx`, ya sin montar | Sensación de **40 °C** con aire a 32,5 · amplitud fina: 1,2 °C |
