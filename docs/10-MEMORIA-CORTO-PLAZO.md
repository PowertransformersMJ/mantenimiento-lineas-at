...
0. 🆕 **PARÁMETROS ELÉCTRICOS** (`§ADR-088..131`). El SCADA entra por los pasos 0·1·2 de `20`, por
   ...
   mín 195), **validados el 11-09** (8 señales, 29/29); lo que falta no vino en la exportación
   (`TODO-101`) y lo apartado espera decisión (`TODO-103`). Pico **588 A el 05-08 = 82 %**, con
...
| **TODO-82/83** | **Clima.** ① ¿FASE 2 del pronóstico: franja y sensación térmica (`§ADR-057`)? ② ¿Dato FINO por extremos (`§ADR-064`)? ③ Satelital «con huecos»: falta DÓNDE | **40 °C** de sensación con aire a 32,5 |
| **TODO-88** | **¿Se junta otra vez el eje del tiempo?** El pronóstico está en el atlas con mes, día y hora (`§ADR-086`) pero en FAMILIA aparte. ¿Lo quiere junto a lo medido en **una tira**? | Medido y modelo juntos hacían «ganar el hecho» |
| **TODO-80** | **¿Qué tope de puesta a tierra rige?** El campo existe (`ADR-052`); declararlo basta. Sin decisión suya, **10 Ω** | Con 18 Ω medidos, 10 Ω dice «revisar» y 25 Ω «cumple» |
...
| **TODO-89** 🔴 ⬅️ **ORDEN SUYA** | **Encender el despliegue automático.** La cadena está cableada y probada (`§ADR-137`); faltan los dos secretos de Cloudflare, que pone él y no pasan por el chat. Sin ellos la corrida DICE que no publicó | **Medido el 25-09:** el vigía rehizo el pronóstico 4 veces bien y producción seguía con uno de **36 h**, espejado y **caducado desde hacía 28** (`§ADR-142`). Cada arreglo de atlas exige despliegue A MANO |
