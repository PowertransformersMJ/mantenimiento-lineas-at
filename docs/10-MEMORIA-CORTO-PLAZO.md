...
3. **EL ATLAS TIENE SEIS CAPAS (`§ADR-079`): sol · temperatura · viento · lluvia · nubes · RAYOS.**
   Los rayos son de OTRA fuente (satélite GOES de NOAA, no NASA POWER) y **se ACUMULAN**: su año son
   0,47 TB, así que cada pasada baja solo lo que falta al libro `herramientas/rayos-conteo.json`.
   ⚠️ **No es la DDT de RETIE/IEEE** —eso es la 2.ª capa, y espera su cuenta Earthdata (`TODO-90`)**.**
...
9. **Higiene** (`99 §ADR-047/048`). ⚠️ El techo es el **ARRANQUE**; `20` y `00` ya pasan su tope.
