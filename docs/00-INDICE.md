...
| `ADR-030` | 2026-08-17 | **La ficha estructural se puede ESCRIBIR.** 0 veredictos no era fallo del cálculo: el motor dictamina desde hace meses y **no había por dónde meter el dato** (`TODO-57`). Entran SEIS campos, la **procedencia se declara por CAMPO** y el **antes/después se enseña ANTES de guardar, sobre el TRAMO ENTERO** —incluidos los que PIERDEN veredicto—. Nada se prerrellena. Deuda peligrosa → cerrada en `ADR-032`; el lote → `ADR-038`. **NO revisada externamente** | *(`tests/ficha-editable.test.js`)* |
