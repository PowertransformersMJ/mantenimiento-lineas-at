...
| `ADR-036` | 2026-08-19 | **La temperatura del suelo pasa de imagen a MEDIDA.** Se veía a cuadros porque lo guardado eran cuadraditos de color; ahora viaja la rejilla de valores (un byte por celda de 30 m) y el color lo pone el navegador. De golpe: **elegir el día** (12 fechas, 3,4 MiB, menos que la única de antes), **leer los grados de un punto con un clic** e interpolación al acercarse. **Byte 0 = SIN DATO** y nubes enmascaradas con el `qa_pixel`: bajo una nube el sensor mide el techo, 20 °C más frío, y saldría como zona fresca. La satelital seguirá borrosa —Sentinel-2 mide a 10 m— y eso se DICE. **NO revisada externamente** | *(`tests/termico.test.js`)* |
| `ADR-035` | 2026-08-19 | **El pronóstico entra como capa, pero no como dato.** Caduca, así que es la ÚNICA pieza del mapa que necesita internet. Fuente **MET Norway** (CC BY 4.0, global, sin clave); Open-Meteo fuera por no comercial. **NO SE GUARDA JAMÁS** —archivarlo lo convertiría en una falsa medición—, se consulta al encender la capa y por una **celda redondeada**, no por la línea. No es un degradado (el modelo trae celdas de km): es el dato del sitio, con la componente de viento que empuja **DE LADO**. Y dice que **no valida la hipótesis**. **NO revisada externamente** | *(`tests/pronostico.test.js`)* |
| `ADR-034` | 2026-08-19 | **Se enciende la satelital y entra la del calor del suelo.** Licencias verificadas con fuente y fecha: Esri, EOX cloudless y Open-Meteo **prohíben el uso comercial** en su vía gratuita — fuera las tres. Se PROCESA el dato abierto y se autohospeda, como el mapa base: Sentinel-2 (Copernicus) y Landsat ST_B10 (USGS, dominio público). Recorte metropolitano: ceñirlo al corredor lo delataría. ⚠️ El térmico es la temperatura del **SUELO en un instante, no del aire**, y **no alimenta ningún cálculo**. **NO revisada externamente** | *(`tests/mapa-capas.test.js`)* |
