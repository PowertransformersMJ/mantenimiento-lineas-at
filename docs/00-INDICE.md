...
| `ADR-019` | 2026-08-04 | **Cero registro público**: el alta de personas es un acto administrativo — `herramientas/usuarios.mjs` es la única vía. RBAC por *claims* leído desde `firestore.rules`; cuentas que se deshabilitan y nunca se borran; cada cambio de rol revoca los tokens. La defensa canónica exige plan de pago: se DETECTA en vez de prevenir, y consta como compromiso. 🟡 **Parcial**: falta retirar Google, el cambio obligatorio y App Check. **NO revisada externamente** | *(sin crudo: los hechos de proveedor van verificados dentro del ADR)* |
| `ADR-037` | 2026-08-19 | **La capa del suelo se cambia por el RECURSO SOLAR**, que a diferencia de aquélla **sí es entrada del cálculo**: la ampacidad usa hoy 1.000 W/m² ADOPTADOS sin fuente local. Trece capas —doce meses y la media del año— del **Global Solar Atlas** (CC BY 4.0, sin cuenta), muestreadas una vez y autohospedadas: **3 KiB**. ⚠️ Se mapea ENERGÍA DIARIA y la norma come IRRADIANCIA INSTANTÁNEA: **no se convierte con una regla de tres**. Se publica la oscilación del año (35 %), que una media esconde. **NO revisada externamente** | *(`tests/radiacion.test.js`)* |
| `ADR-036` | 2026-08-19 | **Una capa de datos se guarda como MEDIDA, no como imagen pintada.** Se veía a cuadros porque lo guardado eran cuadraditos de color; con la rejilla de valores salen de golpe elegir la capa temporal, **leer el valor de un punto con un clic** e interpolación al acercarse — y pesa menos. **Byte 0 = SIN DATO**, nunca cero. (Su capa térmica la reemplazó `ADR-037`; la mecánica se quedó.) La satelital seguirá borrosa —Sentinel-2 mide a 10 m— y eso se DICE. **NO revisada externamente** | *(`tests/rejilla.test.js`)* |
| `ADR-035` | 2026-08-19 | **El pronóstico entra como capa, pero no como dato.** Caduca, así que es la ÚNICA pieza del mapa que necesita internet. Fuente **MET Norway** (CC BY 4.0, global, sin clave); Open-Meteo fuera por no comercial. **NO SE GUARDA JAMÁS** —archivarlo lo convertiría en una falsa medición—, se consulta al encender la capa y por una **celda redondeada**. No es un degradado (el modelo trae celdas de km): es el dato del sitio, con la componente de viento que empuja **DE LADO**. Y dice que **no valida la hipótesis**. **NO revisada** | *(`tests/pronostico.test.js`)* |
| `ADR-034` | 2026-08-19 | **Se enciende la satelital y entra la del calor del suelo.** Licencias verificadas: Esri, EOX cloudless y Open-Meteo **prohíben el uso comercial** en su vía gratuita. Se PROCESA el dato abierto y se autohospeda, como el mapa base: Sentinel-2 (Copernicus) y Landsat (USGS, dominio público). Recorte metropolitano: ceñirlo al corredor lo delataría. **NO revisada** | *(`tests/mapa-capas.test.js`)* |
...
| `ADR-030` | 2026-08-17 | **La ficha estructural se puede ESCRIBIR.** 0 veredictos en los dos ejes no era fallo del cálculo: el motor dictamina desde hace meses y **no había por dónde meter el dato** (`TODO-57`). Entran SEIS campos, cada uno porque sin él no hay veredicto — leído en el motor. **La procedencia se declara por CAMPO y se captura por bloque**: preguntarla una vez mezclaría lo medido con lo leído de una placa. La regla dura vive en el molde: rechaza un valor sin sello y `confirmado_humano` no está — confirmar no es un origen. **El antes/después se enseña ANTES de guardar y sobre el TRAMO ENTERO**, incluidos **los que PIERDEN veredicto**. Nada se prerrellena: resellaría con la fecha de hoy un dato de marzo. Deuda peligrosa → **cerrada en `ADR-032`**. **NO revisada externamente** | *(`tests/ficha-editable.test.js`)* |
