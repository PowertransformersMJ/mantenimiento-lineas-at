...
| `ADR-032` | 2026-08-17 | **Una carga es UN hecho fechado, y ahora lo garantiza el código.** El instante lo sellaba cada documento aparte: coincidían solo dentro del mismo milisegundo (17 % en frío). El plan lo sella UNA vez (`??=`). Producción no cambia; **no se desplegó**. Guardián: de 2 puntos a 9, cae **20 de 20**. **NO revisada** | `research-archive/2026-08-17-medicion-instante-de-carga.json` |
| `ADR-030` | 2026-08-17 | **La ficha estructural se puede ESCRIBIR.** Los apoyos tenían **0 veredictos en los dos ejes** y no era un fallo del cálculo: el motor dictamina desde hace meses y no había **por dónde meter el dato** (`TODO-57`). Entran SEIS campos, cada uno porque sin él un apoyo no puede tener veredicto — leído en el motor. **La procedencia se declara por CAMPO y se captura por bloque**: preguntarla una vez por ficha mezclaría lo medido con lo leído de una placa, y el día de la firma no valen lo mismo. La regla dura no vive en la pantalla sino en el molde: `FichaEstructural` rechaza un valor sin sello antes de salir del navegador, el selector arranca VACÍO y `confirmado_humano` no está — confirmar no es un origen, es un acto posterior. **El antes/después se enseña ANTES de guardar y sobre el TRAMO ENTERO** (ADR-002), contando los que ganan veredicto, **los que lo pierden** y los que pasan a REVISAR con la frase que lo explica: «no es una avería nueva, es lo que ya pasaba y hasta hoy no se podía ver». Tres avisos de geometría en vivo porque el núcleo devuelve `null` **en silencio**. Nada se prerrellena: obligaría a **resellar con la fecha de hoy un dato medido en marzo**. Deuda peligrosa: el ACTA aún no marca los veredictos sobre datos supuestos. **NO revisada externamente** | *(sin comité; evidencia: `tests/ficha-editable.test.js`)* |
| `ADR-029` | 2026-08-17 | **RECORDAR no es PROPONER.** «Cargar» partía del GPX crudo y le volvía a preguntar las cinco cosas de cada punto, **incluida la que había confirmado el día antes**. Se resuelve con `herramientas/decisiones-firmadas.json` —extracto REDACTADO del fixture de la bóveda con **lista blanca** de campos— que la aplicación **solo LEE**: quien puede escribir el recuerdo puede FABRICARLO, y «decidido por usted el 16 de agosto» dejaría de ser comprobable. LIBRO MAYOR: se **apenda**, nunca se pisa, y manda la última fechada; **CARGADO manda sobre FIRMADO**. **El veto de ADR-028 NO se reabre**: sin decisión suya el campo sigue vacío, y las dos que él contesta siempre —cuál de sus puntos es, y si lo aprueba— **no se heredan jamás**. Ningún valor recordado se pinta sin su fecha. Lo que hay que saber: la prueba de no-fuga es una lista NEGRA y se queda corta (la defensa primaria es la lista blanca del generador), y el día que alguien meta ahí una fila **deducida** será un dictamen con sello, peor que el vetado porque parece firmado. **NO revisada externamente** | `research-archive/2026-08-17-workflow-recordar-no-proponer.json` |
| `ADR-028` | 2026-08-17 | **La carga de puntos se hace DESDE la aplicación**, con la sesión y el rol del Ingeniero: la llave maestra deja de hacer falta para el trámite semanal (sigue haciendo falta para roles, correcciones, fotos y líneas nuevas). Clave: **la identidad se acuña en el repositorio y el navegador solo la BUSCA** — una colección de semillas en la base serían dos libros que solo convergen si alguien corre un script. Quinto workspace `importar/` + pestaña «Cargar» solo para admin, sin nada preseleccionado. Nueve vetos del crítico, enteros. Dos FATALES con 1.206 pruebas en verde: la lectura previa que las reglas **deniegan** sobre un documento que aún no existe, y la recarga que **destruía el acuse y el acta** al generarlos. **NO revisada externamente** | `research-archive/2026-08-17-workflow-importar-en-la-app.json` · `…-construir-cargar.json` |
| `ADR-027` | 2026-08-16 | **La identidad de un punto nace de su NOMBRE CANÓNICO**, nunca del índice ni del nombre del GPS (que tiene errores confirmados: el apoyo E07 quedó grabado como «E02»). Las 26 semillas posicionales de julio quedan ancladas en `herramientas/semillas-emitidas.json`, registro que solo crece y vive en el repo PÚBLICO para que la prueba corra en CI — hasta ese día `npm test` seguía verde si los 26 ids cambiaban y dejaban huérfanas 99 fotos y el expediente de la falla. Un punto intercalado entra por **bisección** del `orden`: renumerar habría movido 64 fotos y las habría PISADO. Contrato **0.5.0** (aditivo) y **desplegar antes, sembrar después**. **NO revisada externamente** | `research-archive/2026-08-16-workflow-mapa-identidad-apoyos.json` |
| `ADR-026` | 2026-08-07 | **Varias causas raíz, cinco familias nuevas y la séptima condición.** Todo ADITIVO: ni una clave renombrada, cero migración. `causasRaiz` es lista con el tipo de la cláusula 4 de IEC 62740 (única · múltiple · **contribuyente**, que cambia la probabilidad pero puede no evitarlo); `causaRaiz` queda de solo lectura y `causasDeclaradas()` es el dueño único de la precedencia, con candado que rechaza escribir los dos campos a la vez. Once familias → dieciséis: **protección y control** (sin ella un RCA no podía concluir «falló el relé») + terceros accidentales · acto malicioso · fauna · fuego, que salen de dentro de «vegetación». Nueva barrera de contención. Séptima condición: ninguna hipótesis rival viva y callada, y «no concluyente» cuenta para no fabricar certeza. **NO revisada externamente** | `research-archive/2026-08-06-workflow-rca-vs-iec62740.json` |
...
| `ADR-023` | 2026-08-05 | **Informe GERENCIAL**: documento aparte, no el técnico resumido — derivado entero de funciones ya probadas, aquí no se calcula nada nuevo. El titular de la primera página es lo que NO se puede sostener. **Ocho afirmaciones prohibidas, todas con prueba.** **NO revisada externamente** | `research-archive/2026-08-01-workflow-eje-longitudinal.json` (clave `resultado.gerencial`) |
