...
| `ADR-023` | 2026-08-05 | **Informe GERENCIAL**: documento aparte, no el técnico resumido. Contesta si se puede firmar hoy, qué se manda el lunes, qué queda abierto y qué espera una firma. Aquí NO se calcula nada nuevo, y la lista de límites es la MISMA función que la del técnico — dos papeles de la misma línea no pueden decir cosas distintas. Prohibido con prueba: número de riesgo, precio, plazo, «la línea es segura» y texto de un modelo. El titular es lo que NO se puede sostener. **NO revisada externamente** | `research-archive/2026-08-01-workflow-eje-longitudinal.json` (clave `resultado.gerencial`) |
| `ADR-023` | 2026-08-05 | **Informe gerencial**: 10 secciones DERIVADAS enteras, sin calcular nada nuevo. Titular = lo que NO se puede sostener. Prohibido y probado: número de riesgo, precio, plazo, decir que la línea es segura, y ordenar sin declarar que el orden es criterio. Misma lista de límites que el técnico, con prueba de amarre. Un `lev` sin `puntos` lo tumbaba y 20 pruebas no lo veían: su fixture era más completo que la realidad | `research-archive/2026-08-01-workflow-eje-longitudinal.json` |
