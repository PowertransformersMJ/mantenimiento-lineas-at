...
| `ADR-029` | 2026-08-17 | **RECORDAR no es PROPONER: la pantalla le devuelve lo que él ya firmó, con la fecha pegada.** La pestaña «Cargar» partía del GPX crudo y le volvía a preguntar las cinco cosas de cada punto, **incluida la que había confirmado el día antes**. Se resuelve con un libro de decisiones en el repositorio público —`herramientas/decisiones-firmadas.json`, extracto REDACTADO del fixture de la bóveda con **lista blanca** de campos— que la aplicación **solo LEE**: quien puede escribir el recuerdo puede FABRICARLO, y entonces «decidido por usted el 16 de agosto» deja de ser verificable. Es un LIBRO MAYOR: cambiar de opinión **apenda**, nunca pisa, y manda la última fechada. Precedencia declarada: **CARGADO manda sobre FIRMADO**. **El veto de ADR-028 NO se reabre**: sin decisión suya el campo sigue vacío, y las dos preguntas que él contesta siempre —cuál de sus puntos es, y si lo aprueba— **no se heredan jamás**. Ningún valor recordado se pinta sin su fecha, y cambiar cuesta un clic igual que ratificar. Lo que hay que saber: la prueba de no-fuga es una lista NEGRA y se queda corta —la defensa primaria es la lista blanca del generador—, y el día que alguien meta ahí una fila **deducida** tendremos un dictamen con sello, peor que el vetado porque parece firmado. **NO revisada externamente** | `research-archive/2026-08-17-workflow-recordar-no-proponer.json` |
| `ADR-028` | 2026-08-17 | **La carga de puntos se hace DESDE la aplicación**, con la sesión y el rol del Ingeniero: la llave maestra deja de hacer falta para el trámite de cada semana. Clave del diseño: **la identidad se acuña en el repositorio y el navegador solo la BUSCA** — una colección de semillas en la base serían dos libros de identidad que solo convergen si alguien corre un script, y las fotos acabarían colgando del equivocado. Quinto workspace `importar/` (sin `node:`, se prueba entero) + pestaña «Cargar» solo para admin, con cinco preguntas por punto SIN preseleccionar y el antes/después en cifras antes de que exista ningún botón. Nueve vetos del crítico aceptados enteros. Dos FATALES cazados con 1.206 pruebas en verde: la lectura previa que las reglas **deniegan** sobre un documento que aún no existe, y la recarga que **destruía el acuse y el acta** al generarlos. La llave NO se retira: sigue haciendo falta para roles, correcciones, fotos y líneas nuevas. **NO revisada externamente** | `research-archive/2026-08-17-workflow-importar-en-la-app.json` · `research-archive/2026-08-17-workflow-construir-cargar.json` |
| `ADR-027` | 2026-08-16 | **La identidad de un punto nace de su NOMBRE CANÓNICO**, nunca del índice del array ni del nombre que grabó el GPS (que tiene errores confirmados: el apoyo E07 quedó grabado como "E02"). Las 26 semillas posicionales de julio quedan ancladas en `herramientas/semillas-emitidas.json`, un registro que solo crece y que vive en el repo PÚBLICO para que la prueba de los ids corra en CI, donde la bóveda no está — hasta hoy `npm test` seguía verde si los 26 ids cambiaban y dejaban huérfanas 99 fotos y el expediente de la falla. Un punto intercalado entra por **bisección** (`orden` 2,5 entre el 2 y el 3) en vez de renumerar: correr los `orden` habría movido 64 de las 99 fotos y las habría PISADO sin dejar rastro. Contrato **0.5.0** (aditivo, cero migración) y **orden de operaciones obligatorio: desplegar antes, sembrar después**. Ampliación fechada del 11-12 AGO: 25 estructuras · 24 vanos · 7 tramos; las cifras de julio siguen intactas en sus pruebas. **NO revisada externamente** | `research-archive/2026-08-16-workflow-mapa-identidad-apoyos.json` |
