...
| `ADR-012` | 2026-08-01 | El eje longitudinal (terminal · desequilibrio · rotura), diseñado con 7 agentes y verificado a mano contra el motor | `research-archive/2026-08-01-workflow-eje-longitudinal.json` |
...
| `ADR-016` | 2026-08-03 | El nodo de lecciones partido en madre-índice + hijos por tema; los `L-NN` **no se renumeran** porque los cita el código fuente | `research-archive/2026-08-03-workflow-shard-nodo-30.json` |
...
| `ADR-018` | 2026-08-04 | **Carcasa «El Horizonte»**: piel CLARA sobre esqueleto de 3 columnas — en pantalla oscura un dato que falta se esconde. `amanecer` es inalcanzable si falta un apoyo, con prueba. **NO revisada** | `research-archive/2026-08-04-workflow-critica-carcasa.json` |
| `ADR-019` | 2026-08-04 | **Cero registro público**: el alta de personas es un acto administrativo y `usuarios.mjs` la única vía. RBAC por *claims*; se deshabilita, nunca se borra. 🟡 **Parcial**: falta retirar Google y App Check. **NO revisada** | *(sin crudo)* |
| `ADR-047` | 2026-08-21 | **Mantenimiento integral del cerebro.** 6 lentes con refutador: **28 huecos confirmados, 15 tumbados**. El linter decía SANO con `05` tres olas atrasado, tres lecciones enrutadas al hijo equivocado y el disparador de la auditoría profunda **muerto por construcción**. El nodo del MAPA se parte a `34`. **NO revisada** | `research-archive/2026-08-21-auditoria-cerebro-6-lentes.json` |
| `ADR-046` | 2026-08-21 | **Una capa que se pinta y no se puede APRECIAR.** `ADR-041` (rampa al dato) y `ADR-042` (ver todo el recorte) se aplicaron a la temperatura y **nunca cruzaron** a la radiación. Se propagan, con un solo dueño y guardián que recorre las DOS capas (`34 · L-65`). **NO revisada · verificado en vivo** | *(`tests/radiacion.test.js`)* |
| `ADR-045` | 2026-08-21 | **Atlas solar del Caribe** (`#/sol`): NASA POWER horario de 2026 sobre siete departamentos, con mapa base y vigía propios. **11 de 36 celdas superan los 1.000 W/m² adoptados**, todas al norte. **NO revisada · verificado en vivo** | *(`tests/sol-caribe.test.js`)* |
| `ADR-044` | 2026-08-21 | **El cable de guarda entra como INVENTARIO de daño de operación**, vano a vano (contrato **0.7.0**). El hueco significa **NO CONSTA**, nunca «lo lleva». No entra en ningún cálculo. **NO revisada · verificado en vivo** | *(`tests/cable-de-guarda.test.js`)* |
...
| `ADR-042` | 2026-08-20 | **El mapa como pantalla:** pestaña «Detalle GPS», controles AL LADO y botón «Ver todo el recorte» —el «no veo el gradiente» era el ENCUADRE—. **Revisada por Fable: tres fallos reales** (`30 · L-62`). **Verificado en vivo** | *(`tests/detalle-gps.test.js`)* |
| `ADR-041` | 2026-08-20 | **Corrección de criterio:** la rampa de temperatura se ajusta al dato —una escala fija dejaba el mapa de un color y eso NO era prudencia (`30 · L-61`)— y la satelital sube al nivel 16. **NO revisada · verificado en vivo** | *(`tests/temperatura.test.js`)* |
...
| `ADR-039` | 2026-08-20 | **La temperatura del AIRE entra al mapa** (la del suelo no alimentaba ningún cálculo): 3 KiB, misma licencia que el sol. Es **MEDIA, no extremo**. La EDS adoptada (28 °C) se sostiene: el sitio tiene 27,3. **NO revisada · verificado en vivo** | *(`tests/temperatura.test.js`)* |
...
| `ADR-037` | 2026-08-19 | **La capa del suelo se cambia por el RECURSO SOLAR**, que sí es entrada del cálculo. Trece capas del Global Solar Atlas (CC BY 4.0), 3 KiB. ⚠️ ENERGÍA DIARIA ≠ IRRADIANCIA INSTANTÁNEA: **no se convierte con una regla de tres**. **NO revisada** | *(`tests/radiacion.test.js`)* |
| `ADR-036` | 2026-08-19 | **Una capa de datos se guarda como MEDIDA, no como imagen pintada**: se puede elegir el mes, leer el valor de un punto con un clic y pesa menos. **Byte 0 = SIN DATO**, nunca cero. **NO revisada** | *(`tests/rejilla.test.js`)* |
| `ADR-035` | 2026-08-19 | **El pronóstico entra como capa, pero no como dato**: MET Norway (CC BY 4.0); Open-Meteo fuera por no comercial. **NO SE GUARDA JAMÁS** —archivarlo lo convertiría en una falsa medición— y dice que no valida la hipótesis. **NO revisada** | *(`tests/pronostico.test.js`)* |
| `ADR-034` | 2026-08-19 | **Se enciende la satelital.** Esri, EOX y Open-Meteo prohíben el uso comercial gratis, así que se PROCESA el dato abierto y se autohospeda (Sentinel-2, Landsat). Recorte metropolitano: ceñirlo al corredor lo delataría. **NO revisada** | *(`tests/mapa-capas.test.js`)* |
| `ADR-033` | 2026-08-19 | **El sembrador respeta el cerrojo**: metía `revision: 0` con `merge` y rompía el siguiente guardado con un conflicto inexistente. **NO revisada · sin correr contra la base** | *(`tests/sembrar-mapeo.test.js`)* |
| `ADR-032` | 2026-08-19 | **Un veredicto sobre un dato que nadie verificó lo dice en el papel** (informe, gerencial, CSV y pantalla). Quién decide qué está supuesto es **el MOTOR**, único que sabe qué campos comió cada eje. **NO revisada** | *(`tests/informe.test.js`)* |
...
| `ADR-030` | 2026-08-17 | **La ficha estructural se puede ESCRIBIR.** 0 veredictos no era fallo del cálculo: no había por dónde meter el dato. Seis campos, procedencia por CAMPO y el antes/después sobre el TRAMO ENTERO antes de guardar. Nada se prerrellena. **NO revisada externamente** | *(`tests/ficha-editable.test.js`)* |
...
| `ADR-028` | 2026-08-17 | **La carga de puntos se hace DESDE la aplicación**, con su sesión y su rol: la llave maestra deja de hacer falta. Dos fallos FATALES con 1.206 pruebas en verde. **NO revisada externamente** | `research-archive/2026-08-17-workflow-importar-en-la-app.json` |
| `ADR-027` | 2026-08-16 | **La identidad de un punto nace de su NOMBRE CANÓNICO**, nunca del índice ni del nombre del GPS. Un punto intercalado entra por **bisección** del `orden`: renumerar habría pisado 64 fotos. Contrato **0.5.0**. **NO revisada externamente** | `research-archive/2026-08-16-workflow-mapa-identidad-apoyos.json` |
| `ADR-026` | 2026-08-07 | **Varias causas raíz** (única · múltiple · contribuyente, IEC 62740), todo ADITIVO; `causasDeclaradas()` es el dueño único de la precedencia. Séptima condición de cierre: ninguna hipótesis rival viva y callada. **NO revisada externamente** | `research-archive/2026-08-06-workflow-rca-vs-iec62740.json` |
| `ADR-025` | 2026-08-06 | **RCA contrastado contra IEC 62740**: la fórmula firmable es «contrastado contra», nunca «conforme» — la propia norma dice que no atestigua conformidad. Se declara lo que NO se adopta y por qué. **NO revisada externamente** | `research-archive/2026-08-06-workflow-rca-vs-iec62740.json` |
| `ADR-024` | 2026-08-06 | **La puerta de acceso**: orden fechada en el token + recibo en la base. La contraseña es **HIGIENE, no la frontera** —la frontera son las reglas y el rol—; ante duda se deja pasar. **NO revisada externamente** | *(`tests/acceso.test.js`)* |
| `ADR-023` | 2026-08-05 | **Informe GERENCIAL**: documento aparte, no el técnico resumido — contesta otras preguntas y comparte con él el DUEÑO de la lista de límites. Prohibido decir que la línea es segura. **NO revisada externamente** | *(`tests/gerencial.test.js`)* |
| `ADR-022` | 2026-08-05 | **Herramienta INTERNA, sin cliente ni contrato.** El comité fundador asumió un montaje comercial que nadie verificó y la pizarra lo llamó ocho días el cuello de botella de TODO; el bloqueo REAL no estaba en ninguna lista. Lección `30 · L-42` | `research-archive/2026-07-28-comite-vision-arquitectura.md §J` |
| `ADR-021` | 2026-08-04 | **El cerebro puede mentir con todos los gates en verde**: 18 huecos con `brain:check` verde antes y después. **El linter valida ESTRUCTURA, no VERDAD**, y toda cifra copiada se retira del nodo que no es su dueño. **NO revisada** | `research-archive/2026-08-04-auditoria-documentacion-sesion.json` |
| `ADR-020` | 2026-08-04 | Segmento RCA **fuera** de la línea: la causa raíz cara es la que se REPITE, y dentro de una sola línea ese patrón es invisible. 11 espinas propias; no existe «no aplica». **NO revisada externamente** | `research-archive/2026-08-04-workflow-rca-lineas-at.json` |
